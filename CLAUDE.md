...
- **Card media**: One table, `CardImage`, holds every image -- generated by the bot or uploaded by a person, told apart only by `status`. A card *points* at a row: `Flashcard.front_image` and `back_image` are foreign keys, not files. So "accepted" means "the live card points at it", which cannot disagree with itself the way a separate flag could. Prompts, and any image no card points at, stay in the admin API. History is append-only -- regenerating adds a row, never edits one. Each card can pin its own model, remembered against its `version_group`. Uploads are written as `front_image_upload` / `back_image_upload`; the read side returns a URL.
